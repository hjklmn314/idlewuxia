...
- `wuxia-main.js` 仍拥有配置驱动 HTML 生成和延期 Combat 展示时间轴；DOM 查询、绑定、移动布局和持久化生命周期已下沉至 `src/wuxiaDomAdapter.js`。
- `src/wuxia.css` 是 Wuxia shipping 样式；`src/legacy-shooting.css` 保留为非运输参考，不进入 Web/Android 闭包。
