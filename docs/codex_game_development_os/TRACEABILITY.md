...
| 工作区权威分层 | `tools/lib/workspace-hygiene.mjs` | `workspace_hygiene_manifest.json` + Schema；active authority 仍由 `project_scope.json#shippingFiles` 决定 | reference bytes 禁止 shipping | 407 tracked file hashes、active import graph、29-file before/after closure | HYGIENE-001 done；AUDIT-003 严格视觉/Android 门 blocked |
| 客户端安全与隐私 | CSP + Android source/merged manifest audit + secret/dependency scanner | `security_release_contract.json` + Schema；analytics privacy/retention | 外部 SDK 与开发证据禁止 shipping | negative contract tests + npm audit + debug assemble + real Edge CSP | SEC-001 done；签名/SBOM/真机/商店归 REL-001..003 |
