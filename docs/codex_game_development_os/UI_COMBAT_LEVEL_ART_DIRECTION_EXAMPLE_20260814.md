...
附图所示的顶部区域不通过，具体问题不是“装饰太多”这么简单，而是
信息架构混淆：

- 红绿菱形既像回合进度又像状态点，没有明确的“当前行动者”和“下一行动方向”；
- 中间大型字形没有绑定 `encounterId`、`round` 或 `actorId`，属于不可解释的装饰；
- 左侧竖幅和两侧大面积装饰侵占竖屏安全区，却没有提供决策信息；
- 角色头像没有明确区分我方/敌方、当前/未来/已倒地，也没有声明 `unitId`、`side`、`turnIndex` 的绑定；
- 顶部没有形成“上下文 → 回合顺序 → 状态图例”的层级，玩家需要猜测图形含义。

顶部 HUD 改为三层、最多占可用竖屏高度 18%：

1. `context`：遭遇上下文、回合计数、独立暂停按钮；
2. `turn-order`：左到右表达下一行动方向，当前行动者只突出一个，未来单位降权，倒地单位划掉；
3. `state-legend`：用形状加短标签解释我方/敌方/当前/倒地，颜色不能独立承载语义。

![战斗顶部 HUD 独立方向](H:/MyProjectBack/idlewuxia/outputs/ui_style_examples/UI_COMBAT_TOP_HUD_ART_DIRECTION_EXAMPLE_20260815.png)

顶部 HUD 概念证据：该横向裁切实际为 2172 × 724 px，811,510 bytes，
