...
- T03-00 纠偏后为 129 个实体可达、10 个实体/24 个动作受控休眠、0 个未裁决；135 个非战斗动作仍因没有执行模块而隐藏。
...
`ARCH-001` 六个切片已经完成。当前下一施工顺序：

1. `T03-01`：358/358 全动作前后状态断言。
2. `SAVE-001`、`OBS-001`：存档迁移/恢复与事件回放闭环。
3. `UI-ARCH-001`、`QA-UI-001`、`T05-01`：剩余 UI 职责拆分和 11×3 视觉门禁。
4. `T05-02` 与 ASSET-002..006：自有资产运行链。
5. G7：签名 Release、真机、商店、监控和回滚。
