...
- 本 Slice 2B 历史切片结束时后续服务尚未拆分；NavigationService、EntityInteractionService、ChapterSession 和 UI intent adapter 已在 Slice 3-6 全部完成；
- JavaScript 通过运行时合同和门禁提供类型约束，不等于 TypeScript 编译期类型；
- 本合同单独完成当时不代表 ARCH-001；当前 ARCH-001 已由六个切片关闭，但仍不代表 G4、T05-01 或商业发行完成。
