...
export function createChapterSession(sourceContract, options = {}) {
  // A session owns one immutable definition snapshot. Browser config reloads or
  // external test fixtures cannot silently rewrite active command semantics.
  const contract = clone(sourceContract || {});
  const activeChapter = clone(resolveActiveChapter(contract, options));
  ...
  const initialFlags = Array.isArray(saveState.flags)
    ? saveState.flags
    : (options.initialFlags || contract?.sessionDefaults?.initialFlags || []);
  ...
  function snapshot() {
    ...
    const chapterSnapshot = {
      ...
      nodes: clone(activeChapter?.nodes || []),
      rooms: clone(activeChapter?.rooms || []),
      npcs: clone(activeChapter?.npcs || []),
      interactables: clone(activeChapter?.interactables || []),
  ...
  function detachedCommand(command) {
    return (...args) => {
      ...
      const { snapshot: detachedSnapshot, ...metadata } = result;
      const detachedMetadata = clone(metadata);
      return detachedSnapshot === undefined
        ? detachedMetadata
        : { ...detachedMetadata, snapshot: detachedSnapshot };
    };
  }
  ...
  return {
    snapshot,
    exportSaveState,
    dispatch: detachedCommand(dispatch),
    selectChapterNode: detachedCommand(selectChapterNode),
    selectChapterRoom: detachedCommand(selectChapterRoom),
    selectChapterNpc: detachedCommand(selectChapterNpc),
...
export function summarizeChapterSessionContract(contract) {
